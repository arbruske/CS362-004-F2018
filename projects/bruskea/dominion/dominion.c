...
int stewardCardEffect(int choice1, int choice2, int choice3, int currentPlayer, struct gameState *state, int handPos)
{
  if (choice1 == 1)
  {
    //+2 cards
    //**BUG** drawCard should be called twice, not just once
...
int smithyCardEffect(int currPlayer, struct gameState *state, int pos)
{
  //**BUG** Smithy should add 3 cards to the player's deck however here it adds 4 since we have i <= 3
  for (int i = 0; i <= 3; i++)
  {
    drawCard(currPlayer, state);
  }

  discardCard(pos, currPlayer, state, 0);

  return 0;
}

int adventurerCardEffect(int drawntreasure, int currPlayer, struct gameState *state, int temphand[])
{

  //*BUG* z always initalized to 1
  int z = 1;
  int cardDrawn;
  while (drawntreasure < 2)
  {
    ...
    drawCard(currPlayer, state);
    cardDrawn = state->hand[currPlayer][state->handCount[currPlayer] - 1];
  ...
  while (z - 1 >= 0)
  {
    state->discard[currPlayer][state->discardCount[currPlayer]++] = temphand[z - 1];
    z = z - 1;
  }
  return 0;
}

int ambassadorCardEffect(int choice1, int choice2, struct gameState *state, int currentPlayer, int handPos)
{
  ...
  //**BUG** i <= state->handCount[currentPlayer] instead of the correct way:
...
int cutpurseCardEffect(int currentPlayer, struct gameState *state, int handPos)
{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    return adventurerCardEffect(drawntreasure, currentPlayer, state, temphand);
  ...
  case steward:
    return stewardCardEffect(choice1, choice2, choice3, currentPlayer, state, handPos);
  ...
  case ambassador:
    return ambassadorCardEffect(choice1, choice2, state, currentPlayer, handPos);

  case cutpurse:
    return cutpurseCardEffect(currentPlayer, state, handPos);
